...
import { atualizarMaterialPessoal, criarMaterialComArquivoPessoal, criarModuloPessoal, ErroModulo } from "../src/servidor/modulos";
...
async function principal() {
  ...
  try {
    ...
    for (const caso of casos) await criarMaterialComArquivoPessoal(ana.id, { moduloId: modulo.id, titulo: caso.titulo, descricao: `Arquivo sintético válido de ${caso.titulo.toLowerCase()}.`, minutosEstimados: 10 }, caso.arquivo);
    const materiais = await prisma.recursoConteudo.findMany({ where: { moduloId: modulo.id }, include: { arquivo: true }, orderBy: { titulo: "asc" } });
    ...
    for (const candidato of [
      ...
      await criarMaterialComArquivoPessoal(ana.id, { moduloId: modulo.id, titulo: `Inválido ${Math.random()}`, descricao: "Arquivo que deve ser rejeitado pela validação de segurança.", minutosEstimados: 10 }, candidato)
    ...
    await criarMaterialComArquivoPessoal(ana.id, { moduloId: modulo.id, titulo: "Imagem", descricao: "Título duplicado que não deve gerar arquivo órfão.", minutosEstimados: 10 }, arquivo("outra.png", "image/png", [0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a]))
    ...
    try {
      await criarMaterialComArquivoPessoal(bruno.id, { moduloId: modulo.id, titulo: "Tentativa indevida", descricao: "Outra conta não pode anexar material no módulo alheio.", minutosEstimados: 10 }, arquivo("indevido.png", "image/png", [0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a]));
    } catch (erro) {
      bloqueouOutraConta = erro instanceof ErroModulo && erro.codigo === "NAO_ENCONTRADO";
    }
    afirmar(bloqueouOutraConta, "outra conta não pode enviar arquivo para módulo alheio.");
